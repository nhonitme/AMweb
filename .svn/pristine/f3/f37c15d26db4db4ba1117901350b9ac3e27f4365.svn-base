...
import { Editing, Popup as DxPopup,Pager } from 'devextreme-react/data-grid';
...
import { getStoreKindInfos,createStoreKindInfo,updateStoreKindInfo, exportToExcel, importFromExcel, deleteStoreKindInfos, deleteStoreKindInfo } from "@/api/storeKindAPI";
...
import { deleteAcclistInfo } from "@/api/acclistAPI";
...
export default function StoreKindList({
  ...
  const [gridData, setGridData] = useState<StoreKindInfo[]>([]);
  ...
  const loadStoreKindInfos = async () => {
    try {
      setLoading(true);
  ...
  e.cancel = (async () => {
    try {
      ...
      const result = await deleteStoreKindInfo(id, langCode);
    ...
    try {
      ...
      a.href = url;
      a.download = `${translate('Menu_StoreKindInfo', 'Warehouse Classification Management')}_${new Date().toISOString().replace(/[:.-]/g, '')}.xlsx`;
      ...
      notify(
        translate ? translate('EXPORT_FAILED', 'Export failed') : 'Export failed',
        'error',
        3000
      );
    }
  ...
  const handleImport = async (
      ...
      try {
        if (file) {
          ...
          console.debug('calling importStoreKind with file', file.name, file.size, lang);
          const resp = await importFromExcel(file, langCode);
          console.debug('server import response', resp);
        } else {
          if (method === "overwrite" || method === "add") {
            for (const r of rows) {
              await createStoreKindInfo(r as Partial<StoreKindInfo>);
            }
          } else if (method === "update") {
            for (const r of rows) {
              if (r.STORE_KIND_ID) await updateStoreKindInfo(r as StoreKindInfo);
            }
          }
        }
      } catch (err: any) {
        console.error("Import error", err);
        let msg = translate ? translate('IMPORT_FAILED', 'Import failed') : 'Import failed';
...
const handleToolbarDelete = async () => {
    ...
    try {
      await deleteStoreKindInfos(keys);
  ...
  const content = (
    <>
      {!hideToolbar && (
        <GridToolbar
          title="Store Kind List"
        titleKey="Menu_StoreKindInfo" //Quản lý kho bãi
        gridRef={gridRef}
        onRefresh={loadStoreKindInfos}
      ...
      <div className="data-grid-container">
        <BaseDataGrid<StoreKindInfo>
          ...
          screenCd={mode === "lookup" ? "/module/store-kind-management/lookup" : "/module/store-kind-management"}
          gridId={mode === "lookup" ? "store-kind-lookup-grid" : "store-kind-grid"}
          actionButtons
