...
export interface TablePagePropsWithConfigs<T extends BaseTableItem> extends TablePageProps<T> {
  ...
  serverImport?: boolean // Nếu true, ExcelImportModal sẽ gửi file lên server thay vì xử lý local
}
...
export function TablePage<T extends BaseTableItem>({
  ...
  // Handle sort when clicking header
  ...
  const handleFormSubmit = useCallback(
    async (formData: any) => {
      if (formMode === "add" && onAdd) {
        ...
        const merged = { ...editingItem, ...formData } as any
        if ((merged.CUSTOMER_CD === undefined || merged.CUSTOMER_CD === null || merged.CUSTOMER_CD === "") && merged.id) {
          merged.CUSTOMER_CD = merged.id
        }
        const editResult = await onEdit(merged as T)
  ...
  // Generic delete handler via `onDelete` prop
    ...
    <div className="space-y-0">
      ...
      {onImport && excelImportConfig && (
        <ExcelImportModal
          ...
          onImport={(importedData, method, file) => {
            if (serverImport) {
              // When serverImport is enabled, delegate import logic to outer handler
              onImport(importedData, method, file)
