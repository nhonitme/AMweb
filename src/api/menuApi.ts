import type { MenuItem, MenuApiResponse, MenuTreeNode, MenuTreeNodeApi } from "@/types/menu";
import axios from "./axiosClient";
import { isAxiosError } from "axios";

export async function fetchMenuList(): Promise<MenuItem[]> {
  try {
    const resp = await axios.get<MenuApiResponse>("/menu/list", {
    ...
    if (isAxiosError(err)) {
      const msg = (err.response && (err.response.data as any)?.Message) || err.message;
...
export async function fetchMenuTree(): Promise<MenuTreeNode[]> {
  try {
    const resp = await axios.get<MenuApiResponse>("/menu/tree", {
      headers: { "Content-Type": "application/json" },
    });

    const result = resp.data;
    if (!result || !result.Success) {
      throw new Error(result?.Message || "Failed to fetch menu tree");
    }

    const rawTree = (result.Data as unknown as MenuTreeNodeApi[]) ?? [];
    return rawTree.map(mapMenuTreeNodeApi);
  } catch (err) {
    if (isAxiosError(err)) {
      const msg = (err.response && (err.response.data as any)?.Message) || err.message;
      throw new Error(`Failed to fetch menu tree: ${msg}`);
    }
    throw err;
  }
}
