...
export async function deleteStoreInfo(
  ...
  const resp = await axios.delete(`${BASE_URL}`, {
    params: { STORE_ID: storeId, lang: langCode },
  });
  return resp.data;
}
...
export async function importFromExcel(
  ...
  const url = `${BASE_URL}/import${lang ? `?lang=${encodeURIComponent(lang)}` : ''}`;
...
export async function checkExists(
  ...
  const resp = await axios.get(`${BASE_URL}/CheckExists`, { params: { STORE_ID, STORE_CD } });
  return resp.data;
}