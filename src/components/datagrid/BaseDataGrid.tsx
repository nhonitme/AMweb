...
export function BaseDataGrid<T>({
    ...
    const gridInstanceRef = React.useRef<any>(null);
    ...
    React.useEffect(() => {
        if (!columnSettingState.enabled || !gridInstanceRef.current || !columnSettingState.cachedEditorItems.length) {
            return;
        }

        columnSettingState.syncEditorItemsToComponent(gridInstanceRef.current, columnSettingState.cachedEditorItems);
    }, [
        dataSource,
        columnSettingState.cachedEditorItems,
    ...
    const installGridHelpers = React.useCallback((component: any) => {
        if (!component) {
            return;
        }
    ...
    return (
        <div className="data-grid-container h-full w-full">
            <DataGrid
                ...
                noDataText={translate("NO_DATA", "Không có dữ liệu")}
                keyExpr={keyExpr}
                width="100%"
                height="100%"
                key={lang}
                ...
                onInitialized={(e) => {
                    ...
                    installGridHelpers(e.component);
